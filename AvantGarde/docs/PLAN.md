...
| **Host side genuinely consumes the unused messages.** `Avalonia.Controls.Remote.Server` (in `Avalonia.Controls.dll` 12.1.0) references `ClientViewportAllocatedMessage`, `MeasureViewportMessage`, `KeyEventMessage`, `TextInputEventMessage`, `ScrollEventMessage`, `PointerMoved/PressedEventMessage`. `Avalonia.DesignerSupport` additionally names `RequestViewportResizeMessage` and a `viewportAllocatedMessage` local. | Milestone 4's headline features are real server-side capability, not just declared types. Verified, not assumed. |
...
1. **Viewport negotiation → real fit-to-window.** Send `ClientViewportAllocatedMessage {Width, Height, DpiX, DpiY}` from the preview pane's actual arranged size; handle inbound `RequestViewportResizeMessage`; use `MeasureViewportMessage` for measure-only queries. Today AvantGarde never tells the host how large the preview area is — size comes from locally parsed `d:DesignWidth`/`d:DesignHeight` in `Loading/PreviewFactory.cs`, used only for the dimension labels. Neither reference implementation sends these. This is the marquee feature, and the host demonstrably consumes the messages.
2. **Keyboard + text input forwarding.** `KeyEventMessage` (incl. 12's `PhysicalKey`/`KeySymbol`) and `TextInputEventMessage`. AvaloniaRider forwards **no** key events at all. Extend `Loading/PointerEventMessage.cs` into a general input mapper and resolve its existing `KeyModifiers.Meta → InputModifiers.Windows` TODO.
3. **Scroll forwarding.** `ScrollEventMessage {DeltaX, DeltaY, X, Y}` — wire the preview `ScrollViewer` wheel to the guest, with Ctrl+Wheel reserved for zoom.
4. **Zoom-as-DPI is already correct** (`SendScale` sets `ClientRenderInfoMessage.DpiX/Y = 96 * scale`, giving genuine re-render rather than bitmap upscale). Add: fit-to-window and pixel-perfect 1:1 entries alongside the hardcoded 25–400 % ladder in `ViewModels/PreviewOptionsViewModel.cs`; suppress DPI pushes while a XAML update is in flight (AvaloniaRider's interlock).
5. **FPS limiting via delayed ack.** Withhold `FrameReceivedMessage` to throttle — AvaloniaRider's `doWithFpsLimit` trick; the back-pressure channel already exists.
6. **Build on demand.** AvantGarde never invokes MSBuild; the user must pre-build. Add a "Build" affordance on the `"assembly not found"` error from `DotnetProject.CheckForError()`, reusing the `MsBuildEvaluator` process plumbing.
7. **Shadow copy (opt-in).** Copy `TargetDir` to temp and launch from there so rebuilds don't fight file locks — but keep the **original** working directory so relative asset paths still resolve (AvaloniaRider's documented subtlety). Would let `Projects/BuildWatcher.cs` stop suspending the preview during every build.
...
8. Milestone 4: resize the AvantGarde window and confirm the guest re-renders at the new viewport size (not a scaled bitmap); type into a `TextBox` in the preview and confirm characters appear; wheel-scroll a `ScrollViewer` inside the preview.
